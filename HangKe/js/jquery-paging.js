;(function($){
	...
	var defaults={
		...
		pagingDisplay:"flex",
		pagingBtnPost:false, //最大长度位置true在分页按钮前面，false在最后面
		pagingBtnNum:false, //是否在分页按钮10之前的数字加个零
	}
	$.fn.extend({
		"paging":function(options){
			...
			for(var i=0;i<numLI;i++){
				...
				num++
				if(opts.pagingBtnNum==true){
					if(num>=10){
						$(".paging_btn").append("<span>" + num + "</span>"); 
					}else{
						$(".paging_btn").append("<span>0" + num + "</span>"); 
					}
				}else{
					$(".paging_btn").append("<span>" + num + "</span>"); 
				}
			...
			$(".paging_up,.paging_down").unbind("click").click(function(){//==============
				index = ($(this).index());
			...
			if(opts.pageMax==true){
				if(opts.pagingBtnPost==true){
					if(numLI>=10){
						$(".paging_btn").append("<p class='paging_max'>"+ numLI +"</p>")
					}else{
						$(".paging_btn").append("<p class='paging_max'>0"+ numLI +"</p>")
					}
				}else{
